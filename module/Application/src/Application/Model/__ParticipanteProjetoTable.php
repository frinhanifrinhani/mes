...
class ParticipanteProjetoTable {
    ...
    public function fetchAllProjeto($codParticipante) {
        $select = new Select();
        $select->from(new TableIdentifier('participante_para_projeto'))
                ->columns(array('cod_participante', 'cod_projeto'))
                ->join('projeto', 'projeto.cod_projeto = participante_para_projeto.cod_projeto', 'nome_projeto')
                ->where("participante_para_projeto.cod_participante = {$codParticipante}")
